/**********************************************************************************************************************
This MCC Adwords script checks Google grants accounts for mandatory requirements and logs the results in a Google Spreadsheet
Single Account Version : https://gist.github.com/vhsu/3004945614b46d227275b5b511b71b81
Author : Suisseo (Vincent Hsu)
More Info : https://www.suisseo.ch/en/blog/google-ad-grants-script/

1. Detect Campaigns with more than 15 conversions over the last 30 days has bidding strategy set to 'maximize conversion' to allow bids higher that $2
...
var accountData = {
    ...
    '111-111-2222': {
        ...
        'spreadUrl': 'https://docs.google.com/spreadsheets/d/ACCOUNT2SPREADSHEETKEY/edit#gid=0',
        'alertMails': ['example@gmail.co', 'example2@gmail.co'],
        'branded': ['account1','example','example2']
    },
    '333-333-3333': {
        'accountName': 'ACCOUNT 3',
        'spreadUrl': 'https://docs.google.com/spreadsheets/d/ACCOUNT3SPREADSHEETKEY/edit#gid=0',
        'alertMails': ['account1@r34raccount3.com'],
        'branded': ['account3']
    }
};
...
function main() {
    ...
    accountSelector.executeInParallel("runGrantsCheck", null, JSON.stringify(accountData));
}

function runGrantsCheck(sharedParameter) {

    var account = AdWordsApp.currentAccount().getCustomerId();
    //Logger.log(account);

    var accountFlags = JSON.parse(sharedParameter);
    var SpreadsheetUrl = accountFlags[account].spreadUrl;
    var campaignSums = checkCampaigns(SpreadsheetUrl);
    var lowQSSum = getLowQualityKeywords(SpreadsheetUrl);
    var oneWorderSum = getOneWorders(SpreadsheetUrl, accountFlags[account].branded);
    ...
    access.formatRows([
      ...
      var emailMessageTitle = "Suisseo Grants Report - " + accountFlags[account].accountName + " - " + account;
      var emailMessageBody = "Your Google Grants report for the account : " +  accountFlags[account].accountName + " - " + account +" is ready \n\n";
   ...
   //If you want to execute the script every day but only send the notification once per week : uncomment and chose week day
   // if (new Date().getDay() == 3) {
        sendSimpleTextEmail(emailMessageTitle, accountFlags[account].alertMails, emailMessageBody )
   // }
}
...
function checkCampaigns(SpreadsheetUrl) {
    ...
    while (campaignIterator.hasNext()) {
        ...
        //We need to check if the number of adgroups is greater or equal to 2
        ...
        //Lets check the number of ads in each adGroup
        while (adGroupIterator.hasNext()) {
            ...
            if (adsIterator.totalNumEntities() < 2) {
                ...
                inc[3] += 1;
                //Logger.log("Adgroup : " + currentAdGroupName + " : has less than 2 enabled ads" )
